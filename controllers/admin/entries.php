<?php
class Admin_EntriesController extends GlossarController {
    ...
    public function index_action($page = 0) {
        ...
        if ($this->category_id) {
            $this->category = new GlossarCategory($this->category_id);
            $this->records  = GlossarCategory::Get($this->category_id);
        } else {
            $this->records = GlossarEntry::Load();
        }
        $this->page    = $page;
    ...
    public function display_action($id) {
        $this->record = new GlossarEntry($id);
